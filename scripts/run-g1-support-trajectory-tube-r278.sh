...
tube=(
  --support-trajectory-tube --support-trajectory-tube-margin 0.002
...
PYTHONPATH=python:python/evals "${python_bin}" "${corpus}" \
  "${common[@]}" \
  --output benchmarks/results/floating-g1-r278-tube-dormant-cap8
for horizon in 5 10 25; do
  PYTHONPATH=python:python/evals "${python_bin}" "${corpus}" \
    "${common[@]}" "${tube[@]}" \
    --support-trajectory-tube-preview-ticks "${horizon}" \
    --output "benchmarks/results/floating-g1-r278-tube-hard-h${horizon}-cap8"
done
PYTHONPATH=python:python/evals "${python_bin}" "${corpus}" \
  "${common[@]}" "${tube[@]}" \
  --support-trajectory-tube-preview-ticks 5 \
  --support-trajectory-tube-project-intent \
  --output benchmarks/results/floating-g1-r278-tube-combined-h5-cap8

PYTHONPATH=python:python/evals "${python_bin}" \
  python/evals/g1_support_trajectory_tube_r278.py
