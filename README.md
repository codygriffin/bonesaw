...
- The r270 causal split adds a default-off, URDF-name-derived lower-body
  velocity envelope with independently switchable soft viability and hard
  directional-braking parts. In the policy/physics-free G1 replay, hard-only
  holds the right knee above -8 rad/s and moves first fallback 863→888;
  soft-only moves it only to 864. Their combination reaches the next support
  transition at tick 1108, but the benefit collapses to tick 897 when composed
  with r269. The standalone combined distribution is under the 5 ms p99 gate,
...
- R279 adds an exact discrete backward-reachable DCM box over the authored
  contact schedule with independent observer, intent, soft-task, and hard-row
  switches. R280 qualifies the behavior-positive two-axis soft profile:
  release moves 1108→1234 and root/CoM/foot RMS improve by
  14.35%/14.90%/16.87%. Five CPU-4-pinned repeats are semantically bit-exact,
  but every p99 misses the 5 ms gate (5.226–5.730 ms). Hard rows admit only
...
- R313 adds a Rust-owned continuous landing-request envelope driven by measured
  ...
  default-off negative evidence; the remaining behavior gap is continuous
