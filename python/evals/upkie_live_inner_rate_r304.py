#!/usr/bin/env python3
"""R304 evaluate an optional 250 Hz WBC / 1 kHz plant inner loop.

The browser transport remains 50 Hz.  This is an evaluation-only fast profile;
the hosted/public worker remains the requested 50 Hz WBC over 250 Hz MuJoCo
physics.  The existing 8 N lateral disturbance remains an explicit red
recovery holdout.
"""
...
def evaluate_cases(
    ...
    *,
    ...
    qualification_gates = {
        "fast_eval_rate_contract_is_50_250_1000": _rates(fast_control)
        == {
            "stream_hz": 50,
            "control_hz": 250,
            "physics_hz": 1000,
        ...
        "fast_eval_uses_explicit_balanced_nominal_joint_target": (
    ...
    return {
        ...
        "authority_statement": (
            "R304 is evaluation-only: it exercises a faster inner cadence and an "
            "explicit balanced nominal target without changing the public 250/50 "
            "worker or promoting disturbance-recovery authority."
        ),
        "trace_counts": {
...
def render_markdown(metrics: dict[str, Any]) -> str:
    ...
    return f"""# Upkie live inner-rate evaluation — R304

Status: **{'FAST EVALUATION PASS' if metrics['qualified'] else 'FAST EVALUATION REJECTED'}; public 250/50 cadence unchanged; lateral recovery remains unpromoted.**
...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>R304 inner-rate evaluation</title>
<style>
...
</style></head><body><main><div class="kicker">Bonesaw · R304 · evaluation-only</div><h1>Fast inside.<br>Public stays steady.</h1>
<p class="lede">The browser remains a smooth 50 Hz observer. This optional profile runs five allocation-free Rust WBC solves per visible frame against a 1 kHz MuJoCo consequence loop; the hosted worker remains 50 Hz WBC over 250 Hz physics. Nominal standing qualifies in the fast probe; lateral recovery does not.</p>
...
def run(
    ...
    *,
    ...
    legacy_control = r300.run_case(
        ...
        worker_options={"balanced_nominal_joint_target": False},
    )
