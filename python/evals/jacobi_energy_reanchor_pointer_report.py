#!/usr/bin/env python3
"""Evaluate the rejected r292 Jacobi energy re-anchor pointer experiment."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", default="benchmarks/results/g1-jacobi-energy-reanchor-pointer-r292"
    )
    parser.add_argument(
        "--web-report", default="web/G1_JACOBI_ENERGY_REANCHOR_POINTER_R292.html"
    )
...
def main() -> None:
    ...
    result = {
        "schema": "bonesaw.g1-jacobi-energy-reanchor-pointer-r292.v1",
        "revision": "g1-jacobi-energy-reanchor-pointer-r292",
        ...
        "candidate": {
            "feature": "bonesaw-core/jacobi-energy-reanchor-pointer-experiment",
            "production_default": False,
            "control": "r284 slice-iterator energy re-anchor with split/slice raw-pointer pair kernel",
        },
        "semantic_contract": {
    ...
    metrics_output = output / "g1-jacobi-energy-reanchor-pointer-r292-metrics.json"
    ...
    report += markdown_table(["counter", "R284 control", "R292 candidate", "delta"], counter_rows)
    report += [
        "",
        f"The candidate retires {100.0 * counter_deltas['instructions']:+.3f}% more instructions ({candidate_average['instructions'] - control_average['instructions']:.0f} extra per process), and every paired instruction delta is positive ({', '.join(f'{100.0 * value:+.3f}%' for value in paired_instruction_deltas)}). Its larger dynamic branch count reinforces the rejection.",
        "",
        "## Decision",
        "",
    ...
    report_text = "\n".join(report) + "\n"
    report_output = output / "G1_JACOBI_ENERGY_REANCHOR_POINTER_R292.md"
