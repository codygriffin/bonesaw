...
def _causal_measured_window(states: list[dict[str, Any]]) -> bool:
    ...
    for state in states:
        ...
        if not diagnostics or _diagnostic(state, "physics_observation_exact") < 0.5:
            return False
        # R309 is called before the current five-substep integration window;
        # its exact physics snapshot therefore matches the contact mask fed to
        # that solve (`observed`), not the post-step `physics_contact_active`
        # sample published in the same stream frame.
...
def evaluate(
    ...
    *,
    ...
    qualification_gates = {
        ...
        "semantic_replay_exact": r300._semantic(candidate_disturbed)
        == r300._semantic(candidate_replay),
    }
    promotion_gates = {
...
def render_markdown(metrics: dict[str, Any]) -> str:
    ...
    baseline_disturbed = metrics["baseline_disturbed"]
...
|---|---:|---:|
| terminal tick / boundary | {baseline_disturbed['terminal_tick']} / {baseline_disturbed['terminal_pending'] or '—'} | {disturbed['terminal_tick']} / {disturbed['terminal_pending'] or '—'} |
| MaxIterations ticks | {baseline_disturbed['max_iterations_ticks'] or '—'} | {disturbed['max_iterations_ticks'] or '—'} |
| max dynamics / contact residual | {baseline_disturbed['maximum_dynamics_residual']:.3e} / {baseline_disturbed['maximum_contact_residual']:.3e} | {disturbed['maximum_dynamics_residual']:.3e} / {disturbed['maximum_contact_residual']:.3e} |
| controller p99 µs | {baseline_disturbed['controller_step_us']['p99']:.3f} | {disturbed['controller_step_us']['p99']:.3f} |
...
def render_html(report: str, passed: bool) -> str:
    return "\n".join(
        (
            ...
            "<title>Bonesaw R309 · measured landing phase boundary</title>",
            ...
            f"<h1 class='{'pass' if passed else 'fail'}'>Upkie measured landing phase boundary · R309</h1>",
...
def main() -> int:
    ...
    parser.add_argument(
        "--web-report",
        type=pathlib.Path,
        default=pathlib.Path("web/UPKIE_LIVE_MEASURED_LANDING_R309.html"),
    )
    ...
    args.web_report.write_text(
        render_html(markdown, metrics["qualified_as_bounded_default_off_experiment"]),
        encoding="utf-8",
    )
