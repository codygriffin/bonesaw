...
class G1TerminalBoxWbcPlantEvidenceTests(unittest.TestCase):
    ...
    def test_archived_failure_remains_fail_closed(self) -> None:
        ...
        self.assertEqual(metrics["physics_steps_per_branch"], 5)
        self.assertAlmostEqual(metrics["physics_dt_seconds"], 0.004)
        self.assertAlmostEqual(metrics["wbc_control_dt_seconds"], 0.020)
        self.assertEqual(metrics["foot_probes_per_foot"], 2)
        self.assertIn("primitive", metrics["plant_fixture"])
