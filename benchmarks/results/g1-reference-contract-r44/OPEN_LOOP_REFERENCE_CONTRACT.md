...
| Bonesaw LIPM · matched hard boundary | PASS | 2.17 m/s² | 697.2 m/s³ | 0.89 cm | 0.00% | 0.221 | 0.93 cm | 0.871 m | 0.000 / 0.000 m/s |
| Bonesaw LIPM · robot-reachable | PASS | 1.99 m/s² | 31.8 m/s³ | 0.88 cm | 0.00% | 0.203 | 0.93 cm | 0.800 m | 0.000 / 0.000 m/s |
| Bonesaw LIPM · morphology-rooted | PASS | 1.99 m/s² | 31.8 m/s³ | 0.88 cm | 0.00% | 0.203 | 0.93 cm | 0.777 m | 0.000 / 0.000 m/s |
...
Bonesaw solves two exact constant-CoP LIPM arcs over the matched opening double-support interval. It searches a fixed switch-time grid without heap storage, solves both CoP positions analytically, and admits a candidate only when the first CoP is inside the opening support hull and the second and terminal CoPs are inside the future right-sole hull.

The selected split is `0.180` of the `0.995 s` boundary interval. Its declared CoP margins are `1.429 cm` then `0.880 cm`, with `1.750 cm` at the equilibrium hold.

The official-G1 landing reach is reduced from `0.871` to `0.800 m` without changing landing height; the retarget is authored before any controller executes.

The standalone authored trace passes `9/9` strict gates. Planning records `54.2 µs`; 600 closed-form samples record `95.3 µs`. The measured planner and sample regions make `0` and `0` allocation calls respectively. JSON transport and Python NPZ packaging are outside those regions.

The generator is bitwise repeatable: `True`. It invokes no WBC, IK, policy, state integration, or physics rollout.

Against PlaCo, the Rust reference changes maximum CoM acceleration from `1.528` to `1.988 m/s²` and the minimum CoP margin from `-6.612` to `0.880 cm`. This is an intentionally conservative first-step boundary primitive, not yet a multi-step walking-pattern replacement.
