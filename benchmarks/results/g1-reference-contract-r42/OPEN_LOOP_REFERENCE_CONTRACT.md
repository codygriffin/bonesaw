# Bonesaw R42 open-loop G1 reference contract
...
| Bonesaw LIPM · hard boundary | PASS | 2.17 m/s² | 697.2 m/s³ | 0.89 cm | 0.00% | 0.221 | 0.93 cm | 0.871 m | 0.000 / 0.000 m/s |
...
The selected split is `0.180` of the `0.995 s` boundary interval. Its declared CoP margins are `1.593 cm` then `0.891 cm`, with `1.750 cm` at the equilibrium hold.

The standalone authored trace passes `9/9` strict gates. Planning records `29.3 µs`; 600 closed-form samples record `99.5 µs`. The measured planner and sample regions make `0` and `0` allocation calls respectively. JSON transport and Python NPZ packaging are outside those regions.

The generator is bitwise repeatable: `True`. It invokes no WBC, IK, policy, state integration, or physics rollout.

Against PlaCo, the Rust reference changes maximum CoM acceleration from `1.528` to `2.167 m/s²` and the minimum CoP margin from `-6.612` to `0.891 cm`. This is an intentionally conservative first-step boundary primitive, not yet a multi-step walking-pattern replacement.
